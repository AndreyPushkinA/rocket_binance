...
async fn binance_btc_trades() -> Result<BtcTrades, Error> {
    ...
    for trade in &recent_trades_response {
        if let Err(e) = insert_trade_into_clickhouse(trade).await {
            eprintln!("Error inserting trade data: {}", e);
        }
    }
...
async fn insert_trade_into_clickhouse(trade: &RecentTrade) -> Result<(), reqwest::Error> {
    let client = reqwest::Client::new();
    
    let timestamp = NaiveDateTime::from_timestamp(trade.time / 1000, 0);
